...
def optimize_label_positions_fast(
    ax,
    anchors,
    labels,
    allowed_radii,
    ...
    # Pre-compute label dimensions once
    label_dims = get_label_dimensions(ax, labels)
    ...
    for _ in trange(n_random_iterations, desc="Random Iterations"):
        ...
        # Calculate energies using fast distance-based methods
        point_energies = calc_label_point_energies_fast(
            anchors, label_pos, label_dims, influence_extent
        )
        label_energies = calc_label_label_energies_fast(
            label_pos, label_dims, influence_extent
        )
        crossing_energies = calc_crossing_energies(anchors, label_pos)

        energies = point_energies + label_energies + 10 * crossing_energies
    ...
    if best_label_pos is None and n_labels > 0:
        ...
        point_energies = calc_label_point_energies_fast(
            anchors, best_label_pos, label_dims, influence_extent
        )
        label_energies = calc_label_label_energies_fast(
            best_label_pos, label_dims, influence_extent
        )
        crossing_energies = calc_crossing_energies(anchors, best_label_pos)
        lowest_energy = np.sum(point_energies + label_energies + 10 * crossing_energies)
    elif n_labels == 0:
    ...
    for _ in trange(n_local_iterations, desc="Local Iterations"):
        ...
        for i in random_order:
            ...
            for idx, new_pos in enumerate(all_positions[i]):
                ...
                point_energies = calc_label_point_energies_fast(
                    anchors, current_label_pos, label_dims, influence_extent
                )
                label_energies = calc_label_label_energies_fast(
                    current_label_pos, label_dims, influence_extent
                )
                crossing_energies = calc_crossing_energies(anchors, current_label_pos)

                energy = np.sum(
                    point_energies + label_energies + 10 * crossing_energies
                )
    ...
    best_label_energies = point_energies + label_energies + crossing_energies
